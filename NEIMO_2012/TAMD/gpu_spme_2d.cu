...
namespace _spme_ {
	...
	__global__ void FT_prior(VSPME_2d *vspme) { // 1d block, Q ==> ftd
	...
	__global__ void FT_prior_2d(VSPME_2d *vspme) { // Q ==> ftd
		int tidx = threadIdx.x + blockIdx.x * blockDim.x;
		int tidy = threadIdx.y + blockIdx.y * blockDim.y;
		int nxEachThread = _gpu_dstruct_::_gpu_util_::gpuJob_nEachThread(vspme->Nx, gridDim.x, blockDim.x);
		int nyEachThread = _gpu_dstruct_::_gpu_util_::gpuJob_nEachThread(vspme->Ny, gridDim.y, blockDim.y);
		
		int ith, jth, i, j, k, n;
		double vb, vc, vbc;
		for (ith = 0; ith < nxEachThread; ith++) {
			i = nxEachThread * tidx + ith;
			if (i >= vspme->Nx) break;
		for (jth = 0; jth < nyEachThread; jth++) {
			j = nyEachThread * tidy + jth;
			if (j >= vspme->Ny) break;

			n = (i * vspme->Ny + j) * vspme->Nz;
			if (vspme->bUseHostFFT == 0 || vspme->ftd_host == NULL) {
				//memcpy(vspme->ftd.m + n, vspme->Q.mb.m + n, vspme->Nz * sizeof(double));
				for (k = 0; k < vspme->Nz; k++) vspme->ftd.m[n + k] = vspme->Q.mb.m[n + k];
			}
			else {
				//memcpy(vspme->ftd_host + n, vspme->Q.mb.m + n, vspme->Nz * sizeof(double));
				for (k = 0; k < vspme->Nz; k++) vspme->ftd_host[n + k] = vspme->Q.mb.m[n + k];
			}
		}}
	};
	
	__global__ void FT_post1_2d(VSPME_2d *vspme) { // 2d -- block dimension, ftc ==> FQ
		...
		for (jth = 0; jth < nyEachThread; jth++) {
			...
			//for (j = 0; j < vspme->Ny; j++) {
				...
				*/
				...
				//memcpy(vspme->FQ.m[i].m[j].m, vspme->ftc.m + n, (vspme->nhz + 1) * sizeof(cuDoubleComplex));
				if (vspme->bUseHostFFT == 0 || vspme->ftc_host == NULL) {
					//memcpy(vspme->FQ.mb.m + i * vspme->Nyz + j * vspme->Nz, vspme->ftc.m + n, (vspme->nhz + 1) * sizeof(cuDoubleComplex));
					m = i * vspme->Nyz + j * vspme->Nz;
					fq = vspme->FQ.mb.m + m; ftc = vspme->ftc.m + n;
					for (k = 0; k <= vspme->nhz; k++) fq[k] = ftc[k];
				}
				else {
					//memcpy(vspme->FQ.mb.m + i * vspme->Nyz + j * vspme->Nz, vspme->ftc_host + n, (vspme->nhz + 1) * sizeof(cuDoubleComplex));
					m = i * vspme->Nyz + j * vspme->Nz;
					fq = vspme->FQ.mb.m + m; dftc = vspme->ftc_host + n;
					for (k = 0; k <= vspme->nhz; k++) {fq[k].x = DOUBLE(dftc[k].x); fq[k].y = DOUBLE(dftc[k].y);}
				}
				
				//n += _VSPME<atom>::nhz + 1;
			//}
			
			for (k = 1; k <= vspme->nhz; k++) {
				inz = (k == 0 ? 0 : (vspme->Nz - k));
				vspme->FQ.e(inx, iny, inz)->x = vspme->FQ.e(i, j, k)->x;
				vspme->FQ.e(inx, iny, inz)->y = -vspme->FQ.e(i, j, k)->y;
			}

			// FQ ==> FBCQ
			for (k = 0; k <= vspme->nhz; k++) {
				vc = *(vspme->C.e(i, j, k)); //vc = vspme->C.m[i].m[j].m[k]; 
				if (vc == 0) {
					//memset(vspme->FBCQ.e(i, j, k), 0, sizeof(cuDoubleComplex));
					continue;
				}
				inz = (k == 0 ? 0 : (vspme->Nz - k));
				...
				vspme->FBCQ.e(inx, iny, inz)->x = vbc * fq->x; 
				vspme->FBCQ.e(inx, iny, inz)->y = vbc * fq->y;
			}
			
			// FBCQ ==>ftc
			n = (i * vspme->Ny + j) * (vspme->nhz + 1);
			//memcpy(vspme->ftc.m + n, vspme->FBCQ.m[i].m[j].m, (vspme->nhz + 1) * sizeof(cuDoubleComplex));
			
			if (vspme->bUseHostFFT == 0 || vspme->ftc_host == NULL) {
				//memcpy(vspme->ftc.m + n, vspme->FBCQ.mb.m + i * vspme->Nyz + j * vspme->Nz, (vspme->nhz + 1) * sizeof(cuDoubleComplex));
				m = i * vspme->Nyz + j * vspme->Nz;
				ftc = vspme->ftc.m + n; fq = vspme->FBCQ.mb.m + m;
				for (k = 0; k <= vspme->nhz; k++) ftc[k] = fq[k];
			}
			else {
				//memcpy(vspme->ftc_host + n, vspme->FBCQ.mb.m + i * vspme->Nyz + j * vspme->Nz, (vspme->nhz + 1) * sizeof(cuDoubleComplex));
				m = i * vspme->Nyz + j * vspme->Nz;
				dftc = vspme->ftc_host + n; fq = vspme->FBCQ.mb.m + m;
				for (k = 0; k <= vspme->nhz; k++) {dftc[k].x = double(fq[k].x); dftc[k].y = double(fq[k].y);}
			}
			//n += _VSPME<atom>::nhz + 1;
		}}
	};
	...
	__global__ void IFT_post_2d(VSPME_2d *vspme) { // ftd ==> BCQ
		int tidx = threadIdx.x + blockIdx.x * blockDim.x;
		int tidy = threadIdx.y + blockIdx.y * blockDim.y;
		int nxEachThread = _gpu_dstruct_::_gpu_util_::gpuJob_nEachThread(vspme->Nx, gridDim.x, blockDim.x);
		int nyEachThread = _gpu_dstruct_::_gpu_util_::gpuJob_nEachThread(vspme->Ny, gridDim.y, blockDim.y);
		...
		for (jth = 0; jth < nyEachThread; jth++) {
			...
			if (vspme->bUseHostFFT == 0 || vspme->ftd_host == NULL) {
				...
				for (k = 0; k < vspme->Nz; k++) vspme->BCQ.mb.m[n + k] = vspme->ftd.m[n + k];
			}
			else {
				//memcpy(vspme->BCQ.mb.m + n, vspme->ftd_host + n, vspme->Nz * sizeof(double));
				for (k = 0; k < vspme->Nz; k++) vspme->BCQ.mb.m[n + k] = DOUBLE(vspme->ftd_host[n + k]);
			}
		}}
	};

	//__global__ void IFT_post(VSPME_2d *vspme) { // ftd ==> BCQ
	//	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	//	if (tid == 0) {
	...
	__host__ void vspme_FT_IFT(VSPME_2d *vspme_host, VSPME_2d *vspme_dev) {
		...
		int nBlock = 128;
		dim3 nGrid(0, 0);
		_gpu_dstruct_::_gpu_util_::gpu_grid(vspme_host->Nx, nBlock, nGrid.x, 64);
		_gpu_dstruct_::_gpu_util_::gpu_grid(vspme_host->Ny, nBlock, nGrid.y, 64);

		cufftSetStream(vspme_host->ft_plan, cuStream);
		cufftSetStream(vspme_host->ift_plan, cuStream);

		FT_prior_2d<<< nGrid, nBlock, 0, cuStream >>>(vspme_dev);
		...
		FT_post1_2d<<< nGrid, nBlock, 0, cuStream >>>(vspme_dev);
		//cudaStreamSynchronize(cuStream);
		
		// cuFFT is host-API library
		CUFFT_ExecZ2D(vspme_host->ift_plan, vspme_host->ftc.m, vspme_host->ftd.m);
		//cudaStreamSynchronize(cuStream);
		IFT_post_2d<<< nGrid, nBlock, 0, cuStream >>>(vspme_dev);
...
*/
	...
	__host__ double cal_U_recp(VSPME_2d *vspme_host, VSPME_2d *vspme_dev) {
		...
		if (vspme_host->bUseHostFFT == 0 || vspme_host->ftd_host == NULL) {
			E = _gpu_dstruct_::_gpu_util_::SUM_ARRAY2DD_CUML(vspme_host->Q.mb.m, vspme_host->ftd.m, vspme_host->ftd.n, vspme_host->dbuff_dev, vspme_host->dbuff_host, vspme_host->ndbuff);
		}
		else {
			E = _gpu_dstruct_::_gpu_util_::SUM_ARRAY2Dd_CUML(vspme_host->Q.mb.m, vspme_host->ftd_host, vspme_host->ftd.n, vspme_host->dbuff_dev, vspme_host->dbuff_host, vspme_host->ndbuff);
		}
		E *= PI2 / vspme_host->V;
		return E;
	};
	...
	// TQ ==> FTQ for stress tensor calculation
#ifdef __USE_OLD_CODE__
	__global__ void ST_FT_prior_2d(VSPME_2d *vspme) { // TQ ==> ftd
		int tidx = threadIdx.x + blockIdx.x * blockDim.x;
		int tidy = threadIdx.y + blockIdx.y * blockDim.y;
		int nxEachThread = _gpu_dstruct_::_gpu_util_::gpuJob_nEachThread(vspme->Nx, gridDim.x, blockDim.x);
		int nyEachThread = _gpu_dstruct_::_gpu_util_::gpuJob_nEachThread(vspme->Ny, gridDim.y, blockDim.y);
		...
		for (jth = 0; jth < nyEachThread; jth++) {
			...
			n = (i * vspme->Ny + j) * vspme->Nz;
			if (vspme->bUseHostFFT == 0 || vspme->ftd_host == NULL) {
				//memcpy(vspme->ftd.m + n, vspme->TQ.mb.m + n, vspme->Nz * sizeof(double));
				for (k = 0; k < vspme->Nz; k++) vspme->ftd.m[n + k] = vspme->TQ.mb.m[n + k];
			}
			else {
				//memcpy(vspme->ftd_host + n, vspme->TQ.mb.m + n, vspme->Nz * sizeof(double));
				for (k = 0; k < vspme->Nz; k++) vspme->ftd_host[n + k] = double(vspme->TQ.mb.m[n + k]);
			}
		}}
	};
	
	__global__ void ST_FT_post_2d(VSPME_2d *vspme) { // 2d -- block dimension, ftc ==> FTQ
		// FT have half dimension only
		int tidx = threadIdx.x + blockIdx.x * blockDim.x;
		int tidy = threadIdx.y + blockIdx.y * blockDim.y;
		int nxEachThread = _gpu_dstruct_::_gpu_util_::gpuJob_nEachThread(vspme->Nx, gridDim.x, blockDim.x);
		...
		for (jth = 0; jth < nyEachThread; jth++) {
			...
			//for (j = 0; j < vspme->Ny; j++) {
				...
				*/
				...
				n = (i * vspme->Ny + j) * (vspme->nhz + 1);
				//memcpy(vspme->FTQ.m[i].m[j].m, vspme->ftc.m + n, (vspme->nhz + 1) * sizeof(cuDoubleComplex));
				if (vspme->bUseHostFFT == 0 || vspme->ftc_host == NULL) {
					//memcpy(vspme->FTQ.mb.m + i * vspme->Nyz + j * vspme->Nz, vspme->ftc.m + n, (vspme->nhz + 1) * sizeof(cuDoubleComplex));
					m = i * vspme->Nyz + j * vspme->Nz;
					fq = vspme->FTQ.mb.m + m; ftc = vspme->ftc.m + n;
					for (k = 0; k <= vspme->nhz; k++) fq[k] = ftc[k];
				}
				else {
					//memcpy(vspme->FTQ.mb.m + i * vspme->Nyz + j * vspme->Nz, vspme->ftc_host + n, (vspme->nhz + 1) * sizeof(cuDoubleComplex));
					m = i * vspme->Nyz + j * vspme->Nz;
					fq = vspme->FTQ.mb.m + m; dftc = vspme->ftc_host + n;
					for (k = 0; k <= vspme->nhz; k++) {fq[k].x = dftc[k].x; fq[k].y = dftc[k].y;}
				}
			...
			for (k = 1; k <= vspme->nhz; k++) {
				inz = (k == 0 ? 0 : (vspme->Nz - k));
				//if (inx == i && iny == j && inz == k) continue;
				vspme->FTQ.e(inx, iny, inz)->x = vspme->FTQ.e(i, j, k)->x;
				vspme->FTQ.e(inx, iny, inz)->y = -vspme->FTQ.e(i, j, k)->y;
			}
		}}
	};
	...
	__host__ void cal_FTQ(VSPME_2d *vspme_host, VSPME_2d *vspme_dev) {
		...
		ST_FT_prior_2d<<< nGrid, nBlock, 0, cuStream >>>(vspme_dev);
		//cudaStreamSynchronize(cuStream);
		// cuFFT is host-API library
		CUFFT_ExecD2Z(vspme_host->ft_plan, vspme_host->ftd.m, vspme_host->ftc.m); // ftd ==FFT==> ftc, double ==> cuDoubleComplex
		//cudaStreamSynchronize(cuStream);

		ST_FT_post_2d<<< nGrid, nBlock, 0, cuStream >>>(vspme_dev);
		//cudaStreamSynchronize(cuStream);
	...
	// use CPU-FFTW
	__host__ void cal_FTQ_prior(VSPME_2d *vspme_host, VSPME_2d *vspme_dev) {
		cudaStream_t cuStream;
		cudaStreamCreate(&cuStream);
	...
	__host__ void cal_FTQ_post(VSPME_2d *vspme_host, VSPME_2d *vspme_dev) {
		cudaStream_t cuStream;
		cudaStreamCreate(&cuStream);
	...
	__global__ void ST_FT_prior_2d(VSPME_2d *vspme) { // TQ ==> ftd
		...
		for (ith = 0; ith < nEachThread; ith++) {
			...
			if (vspme->bUseHostFFT == 0 || vspme->ftd_host == NULL) vspme->ftd.m[n] = vspme->TQ.mb.m[n];
			else vspme->ftd_host[n] = vspme->TQ.mb.m[n];
		}
	};
	...
	__global__ void ST_FT_post_2d(VSPME_2d *vspme) { // 2d -- block dimension, ftc ==> FTQ
		...
		for (ith = 0; ith < nEachThread; ith++) {
			...
			if (vspme->bUseHostFFT == 0 || vspme->ftc_host == NULL) {
				...
				fq->x = ftc->x; fq->y = ftc->y;
				if (k != 0) {
					vspme->FTQ.e(inx, iny, inz)->x = ftc->x;
					vspme->FTQ.e(inx, iny, inz)->y = -ftc->y;
				}
			}
			else {
				fq = vspme->FTQ.mb.m + m; dftc = vspme->ftc_host + n;
				fq->x = DOUBLE(dftc->x); fq->y = DOUBLE(dftc->y);
				if (k != 0) {
					vspme->FTQ.e(inx, iny, inz)->x = DOUBLE(dftc->x);
					vspme->FTQ.e(inx, iny, inz)->y = -DOUBLE(dftc->y);
				}
			}
		}
	};
	...
	__host__ void cal_FTQ(VSPME_2d *vspme_host, VSPME_2d *vspme_dev) {
		...
		ST_FT_prior_2d<<< gridDim, blockDim, 0, cuStream >>>(vspme_dev);
		//cudaStreamSynchronize(cuStream);
		// cuFFT is host-API library
		CUFFT_ExecD2Z(vspme_host->ft_plan, vspme_host->ftd.m, vspme_host->ftc.m); // ftd ==FFT==> ftc, double ==> cuDoubleComplex
		//cudaStreamSynchronize(cuStream);
		...
		// the job assigned with half of Nz, or full dimension : Nx * Ny * (nhz + 1)
		ST_FT_post_2d<<< gridDim_hv, blockDim, 0, cuStream >>>(vspme_dev);
		//cudaStreamSynchronize(cuStream);
	...
	__global__ void cal_ST_recp_0_kernel(VSPME_2d* vspme, int iST, DOUBLE *s_block) {
		if (iST != 1 && iST != 2) return; // only for xx and yy
	...
	__global__ void cal_ST_recp_zz_kernel(VSPME_2d* vspme, DOUBLE *s_block) {
		int tid = threadIdx.x + blockIdx.x * blockDim.x;
		int nEachThread = _gpu_dstruct_::_gpu_util_::gpuJob_nEachThread(vspme->Nv, gridDim.x, blockDim.x);
		...
		int ith, i, j, k, n, tn;
		int inx, iny, inz;
		int Nx = vspme->Nx, Ny = vspme->Ny, Nz = vspme->Nz, nhz = Nz / 2;

		_gpu_dstruct_::FLEX_CMATRIX<DOUBLE> *c = &(vspme->Czz);
		DOUBLE vc, vb, vbc, st;
		...
		for (ith = 0; ith < nEachThread; ith++) {
			...
			if (vc == 0) continue;
			vb = vspme->b[0].m[i] * vspme->b[1].m[j] * vspme->b[2].m[k]; vbc = vc * vb;
			fq = vspme->FQ.e(i, j, k); ftq = vspme->FTQ.e(i, j, k);
	...
	__host__ double cal_ST_recp_0(VSPME_2d *vspme_host, VSPME_2d* vspme_dev, int iST) {
		int blockDim = 256;
		...
		if (iST == 3) cal_ST_recp_zz_kernel<<< gridDim, blockDim, 512, cuStream >>>(vspme_dev, vspme_host->dbuff_dev); // zz
		else cal_ST_recp_0_kernel<<< gridDim, blockDim, 512, cuStream >>>(vspme_dev, iST, vspme_host->dbuff_dev); // xx or yy
	...
	__global__ void prior_init_normalized_coordinates_kernel(_VSPME_2d *vspme) {
	...
	__global__ void init_normalized_coordinates_kernel(VSPME_2d *vspme) {
		int tid = threadIdx.x + blockIdx.x * blockDim.x;
		...
		for (i = 0; i < nEachThread; i++) {
			...
			// z is not periodical anymore
			if (u[i3 + 2] < 0 || u[i3 + 2] >= Nz) continue;
	...
	__global__ void VSPME_Q_accum(VSPME_2d *vspme, bool bQ, bool bMU) {
		// work on one cubic of the meshed cell, gridDim is [Nx, Ny] while blockDim is Nz
		int icx = blockIdx.x;
		int icy = blockIdx.y;
		int icz = threadIdx.x;
		...
		int nb = bsp_indx;
		double d[3], md[3] = {vspme->md[0], vspme->md[1], vspme->md[2]};
		int Nx = vspme->Nx, Ny = vspme->Ny, Nz = vspme->Nz;
		...
		for (i = 0; i < nb; i++) {
			...
			for (j = 0; j < nb; j++) {
				...
				for (k = 0; k < nb; k++) {
					...
					if (kz < 0 || kz >= Nz) continue; // z axis is not periodical anymore
	...
	__global__ void prior_Q_accum_kernel(VSPME_2d *vspme) {
		// work on one cubic of the meshed cell, gridDim is [Nx, Ny] while blockDim is Nz
		int icx = blockIdx.x;
		int icy = blockIdx.y;
		int icz = threadIdx.x;
	...
	__global__ void sum_Q_kernel(VSPME_2d *vspme) {
		// work on one cubic of the meshed cell, gridDim is [Nx, Ny] while blockDim is Nz
		int icx = blockIdx.x;
		int icy = blockIdx.y;
		int icz = threadIdx.x;
...
***************************************************************************************/
	...
	__global__ void EF_recp(VSPME_2d *vspme, bool bE, bool bF, bool bQ, bool bMU) { // E of atom is accumulated ,without initilized
		...
		for (iatom = 0; iatom < nEachThread; iatom++) {
			...
			for (i = -nb; i <= 0; i++) {
				...
				for (j = -nb; j <= 0; j++) {
					...
					//for (k = -nb; k <= 1; k++) {
					for (k = -nb; k <= 0; k++) {
						kz = nz + k; if (kz < 0 || kz >= Nz) continue; // z-axis is not periodical anymore
						duz = uz - kz; //if (duz < 0 || duz >= bsp_indx) continue;
...
*************************************************************************************************/
#ifdef __DOUBLE_PRECISION__
	__global__ void VSPME_TQ_accum(VSPME_2d *vspme, bool bQ, bool bMU) {
		// work on one cubic of the meshed cell, gridDim is [Nx, Ny] while blockDim is Nz
		int icx = blockIdx.x;
		int icy = blockIdx.y;
		int icz = threadIdx.x;
		...
		DOUBLE *TQ = vspme->TQ.e(icx, icy, icz);
		TQ[0] = 0;
			...
			for (j = 0; j < nb; j++) {
				...
				for (k = 0; k < nb; k++) {
					...
					for (iatom = 1; iatom <= nc; iatom++) {
						...
						if (bQ) {
							TQ[0] += qz * Mn[0] * Mn[1] * Mn[2]; // accumulating
						}
						if (bMU && mMP > 0) {
							// accumulating
							TQ[0] += z * (mu[0] * dMn[0] * Mn[1] * Mn[2] / md[0] + mu[1] * Mn[0] * dMn[1] * Mn[2] / md[1] + mu[2] * Mn[0] * Mn[1] * dMn[2] / md[2]);
							TQ[0] += mu[2] * Mn[0] * Mn[1] * Mn[2]; // accumulating
						}
					}
				}
			}
	...
	__host__ void cal_TQ_spme(VSPME_2d *vspme_host, VSPME_2d *vspme_dev, bool bQ, bool bMU) {
		...
		cudaStream_t cuStream;
		cudaStreamCreate(&cuStream);
		VSPME_TQ_accum<<<dimGrid, dimBlock, 0, cuStream>>>(vspme_dev, bQ, bMU);
		cudaStreamSynchronize(cuStream);
		cudaStreamDestroy(cuStream);
	};

#else
	__global__ void prior_TQ_accum_kernel(VSPME_2d *vspme) {
		// work on one cubic of the meshed cell, gridDim is [Nx, Ny] while blockDim is Nz
		int icx = blockIdx.x;
		int icy = blockIdx.y;
		int icz = threadIdx.x;
		...
		DOUBLE *Q;// = vspme->TQ.e(icx, icy, icz); Q[0] = 0;
		for (i = 0; i < Nflows; i++) {
			Q = vspme->TQbuf[i].e(icx, icy, icz); Q[0] = 0;
		}
	};
	__global__ void sum_TQ_kernel(VSPME_2d *vspme) {
		// work on one cubic of the meshed cell, gridDim is [Nx, Ny] while blockDim is Nz
		int icx = blockIdx.x;
		int icy = blockIdx.y;
		int icz = threadIdx.x;
		...
		DOUBLE *Qbuf = NULL;
		for (i = 0; i < Nflows; i++) {
			Qbuf = vspme->TQbuf[i].e(icx, icy, icz); Q[0] += Qbuf[0];
		}
	};
			...
			for (i = 0; i < nb; i++) {
				...
				for (j = 0; j < nb; j++) {
					...
					for (k = 0; k < nb; k++) {
						...
						//TQ = vspme->TQ.e(kx, ky, kz);
						TQ = vspme->TQbuf[iflow].e(kx, ky, kz);

						TQa = 0;
						if (bQ) {
							TQa += qz * Mn[0] * Mn[1] * Mn[2]; // accumulating
						}
						if (bMU && mMP > 0) {
							// accumulating
							TQa += z * (mu[0] * dMn[0] * Mn[1] * Mn[2] / md[0] + mu[1] * Mn[0] * dMn[1] * Mn[2] / md[1] + mu[2] * Mn[0] * Mn[1] * dMn[2] / md[2]);
							TQa += mu[2] * Mn[0] * Mn[1] * Mn[2]; // accumulating
						}

						atomicAdd(TQ, DOUBLE(TQa));
					}
				}
			}
		}
	...
	__host__ void cal_TQ_spme(VSPME_2d *vspme_host, VSPME_2d *vspme_dev, bool bQ, bool bMU) {
		...
		dim3 dim3Grid(vspme_host->Nx, vspme_host->Ny);
		int dimBlock = vspme_host->Nz;
		prior_TQ_accum_kernel<<< dim3Grid, dimBlock, 0, cuStream >>>(vspme_dev);
		//cudaStreamSynchronize(cuStream);
		...
		//dim3Grid(vspme_host->Nx, vspme_host->Ny);
		dimBlock = vspme_host->Nz;
		sum_TQ_kernel<<< dim3Grid, dimBlock, 0, cuStream >>>(vspme_dev);
		cudaStreamSynchronize(cuStream);
	...
	__global__ void _init_device_spme(VSPME_2d *vspme) {
	...
	__global__ void init_device_spme(VSPME_2d* vspme) {
		int tid = threadIdx.x + blockDim.x * blockIdx.x;
		if (tid == 0) {
	...
	__host__ bool setup_vspme_host(VSPME_2d *vspme_host, VSPME_2d** vspme_dev, bool b_init, float *status_host, 
		int bsp_indx, double *bsp_x, double *bsp_y0, double *bsp_y1, double *bsp_y2, int bsp_npts, 
		float *xl, float *xd, double kappa, int Nx, int Ny, int Nz, int nBSP,
		short mMP, double *r_hostmap, double *q_hostmap, double *mu_hostmap, double *E_hostmap, double *F_hostmap, int Na) {
		...
		//cudaMemGetInfo(&size_free, &size_total);
			...
			vspme_host->set_cell(xl[0], xl[1], xl[2], xd[0], xd[1], xd[2], Nx, Ny, Nz);
			/*
			vspme_host->Nx = Nx;
			vspme_host->Ny = Ny;
			vspme_host->Nz = Nz;
		...
		cudaMalloc(vspme_dev, sizeof(VSPME_2d));
		_init_device_spme<<<1, 1, 0, cuStream>>>(*vspme_dev);
		cudaStreamSynchronize(cuStream);
		
		cudaMemcpy(*vspme_dev, vspme_host, sizeof(VSPME_2d), cudaMemcpyHostToDevice);
		...
		size_total = sizeof(VSPME_2d);
		cudaMemcpy(vspme_host, *vspme_dev, sizeof(VSPME_2d), cudaMemcpyDeviceToHost);
