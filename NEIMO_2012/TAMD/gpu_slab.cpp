...
void GPU_SLAB_MD_PROC(MMOL_MD_CELL &mmcell, CMM_CELL3D<BASIC_CLUSTER> *cmm) {
	...
	//ecell.k0var.iExIndDipole = (::bExcludeInducedDipole ? 1 : 0);
...
if (tsave.bsave()) (*tsave.out)<<nloop<<"  ";
		...
		if (ecell.gpu_cell.srnb_dev == NULL && ecell.bCalUseGPU) {
			...
			_atomic_cmm_::gpu_reset_esr_neighbors(&ecell);
		...
		if (bImplicitSolvent) {
			show_log("ERROR: Implicit solvent interaction is not realized in GPU calculation.", true);
		}
